...
<body>
	<fd:tabs id="tt" tabPosition="top" fit="false">
		<fd:tab id="baseInfo" title="{com.glaway.ids.pm.project.plan.baseinfo}"
			href="taskFlowTemplateController.do?goTab&tabIndex=1&cellId=${cellId}&templateId=${templateId}&isStandard=${isStandard}&refDuration=${refDuration}&isEnableFlag=${isEnableFlag}&isAjax=true">
		</fd:tab>
		<%-- <fd:tab id="cellInput" title="{com.glaway.ids.pm.project.plan.input}" 
			href="taskFlowTemplateController.do?goTab&tabIndex=2&cellId=${cellId}&templateId=${templateId}&isEnableFlag=${isEnableFlag}&isAjax=true">
		</fd:tab>
		<fd:tab id="cellOutput" title="{com.glaway.ids.pm.project.plan.output}"
			href="taskFlowTemplateController.do?goTab&tabIndex=3&cellId=${cellId}&templateId=${templateId}&isEnableFlag=${isEnableFlag}&isEnableFlag=${isEnableFlag}&isAjax=true">
		</fd:tab> --%>
	</fd:tabs>
</body>
...
<script type="text/javascript">
	...
	function openAdd(dgId, url, baseInfoId, id) {
		...
		$("#" + id).lhgdialog("open",
				'url:' + url + "&ids=" + ids + "&baseInfoId=" + baseInfoId);
	}
	...
	function onClickRowBatch(index, row) {
		if (editIndex != index) {
			if (endEditing('batchModifyEditList')) {
				$('#batchModifyEditList').datagrid('selectRow', index)
	...
	function saveChanges() {
		...
		if (ids.length == 0) {
			...
			top
					.tip('<spring:message code="com.glaway.ids.pm.project.task.saveSuccess"/>');
			return;
		}
