...
<body>
	...
	<div border="false" id="changeInfo" class="easyui-panel div-msg div-msg-fit" fit="true" style="width:100%;height:100%;">
		<fd:form id="planChangeForm">
			...
			<input id="typeIds" name="typeIds" type="hidden" value="${typeIds}" />
											...
											<c:if test="${list.size() == 1 && list.get(0).get(0).control!='8'}">
												...
												<c:if test="${list.get(0).get(0).control=='4'}"> <!-- 日期文本框 -->
													<div id="${list.get(0).get(0).propertyValue}Div">
														...
														<c:choose>
															<c:when test="${list.get(0).get(0).readWriteAccess==enterType || list.get(0).get(0).readWriteAccess=='1'}">
																<fd:inputDate id="${list.get(0).get(0).id}"
																			  editable="false" required="${list.get(0).get(0).required}" title="${list.get(0).get(0).propertyName}" value="${list.get(0).get(0).defaultValue}" ></fd:inputDate>
															</c:when>
															<c:otherwise>
																<fd:inputDate id="${list.get(0).get(0).id}" readonly="true"
																			  editable="false" required="${list.get(0).get(0).required}" title="${list.get(0).get(0).propertyName}" value="${list.get(0).get(0).defaultValue}" ></fd:inputDate>
															</c:otherwise>
														</c:choose>
															<%--</c:if>--%>
														...
														<c:choose>
															<c:when test="${list.get(0).get(0).readWriteAccess==enterType || list.get(0).get(0).readWriteAccess=='1'}">
																<fd:combobox id="${list.get(0).get(0).id}" title="${list.get(0).get(0).propertyName}"  name="${list.get(0).get(0).propertyName}" required="${list.get(0).get(0).required}" url="${list.get(0).get(0).loadUrl}" panelMaxHeight="200"
																			 readonly="${list.get(0).get(0).readOnly}" valueField="${list.get(0).get(0).valueField}" textField="${list.get(0).get(0).textField}" prompt="" selectedValue="${list.get(0).get(0).valueInfo}">
																</fd:combobox>
															</c:when>
															<c:otherwise>
												...
												<c:if test="${list.get(0).get(0).control=='5'}"> <!-- 下拉框 -->
													<div id="${list.get(0).get(0).propertyValue}Div">
														...
														<c:choose>
															<c:when test="${list.get(0).get(0).readWriteAccess==enterType || list.get(0).get(0).readWriteAccess=='1'}">
																<fd:combobox id="${list.get(0).get(0).id}" title="${list.get(0).get(0).propertyName}"  name="${list.get(0).get(0).propertyName}" required="${list.get(0).get(0).required}" url="${list.get(0).get(0).loadUrl}" panelMaxHeight="200"
																			 editable="false" readonly="${list.get(0).get(0).readOnly}" valueField="${list.get(0).get(0).valueField}" textField="${list.get(0).get(0).textField}" prompt="" selectedValue="${list.get(0).get(0).valueInfo}">
																</fd:combobox>
															</c:when>
															<c:otherwise>
																<fd:combobox id="${list.get(0).get(0).id}" title="${list.get(0).get(0).propertyName}"  name="${list.get(0).get(0).propertyName}" readonly="true"  url="${list.get(0).get(0).loadUrl}" panelMaxHeight="200"
	...
	<fd:dialog id="preposePlanDialog" width="830px" height="580px"
	...
	<fd:dialog id="openDocumentInfoDialog" width="870px" height="550px" modal="true" title="文档详情">
		<fd:dialogbutton name="{com.glaway.ids.common.btn.close}" callback="hideDiaLog"></fd:dialogbutton>
	</fd:dialog>
	...
	<script>
        ...
        */
		...
		function start() {
			...
			var changeRemark = $('#changeRemark').val();
			$('#changeRemarkAfter').textbox("setText", changeRemark);
		...
		function savePreposePlan(preposeIds, preposePlans, preposeEndTime) {
			...
			if (preposeEndTime != null && preposeEndTime != '' && preposeEndTime != undefined) {
					...
					success : function(data) {
						...
						if (data != null) {
							...
							if (d.success == true) {
								...
								$('#Plan-planStartTime').datebox("setValue", preposeEndTime);
							} else {
							}
						} else {
						}
				...
				$('#preposeEndTime').val('');
				$('#Plan-planStartTime').datebox("setValue", $("#parentStartTime").val());
			}
		}
		...
		function initChangeVo() {
			...
			var changeInfoDocPath = $('#changeInfoDocPath').val();
			var preposeIds = $('#preposeIds').val();
			var preposePlans = $('#Plan-preposeIds').val();
		...
		function afterSubmitChangeInfo(file, data, response) {
			...
			if (size > 5) {
				...
				var preposeIds = $('#Plan-preposeIds').val();
				var changeRemark = $('#changeRemark').val();
				var changeType = $('#changeType').combotree('getValue');
				//var changeType = $('#changeType').combobox('getValue');
				var changeInfoDocPath = $('#changeInfoDocPath').val();
		...
		function startPlanChange() {
			...
			$.ajax({
				url : 'planChangeController.do?startPlanChange&changeType=' + 'single'+'&leader='+leader+'&deptLeader='+deptLeader+'&typeIds='+typeIds,
				type : 'post',
				data : {
					'leader' : leader,
		...
		function checkPlanWithInput() {
			...
			if (planName == undefined) {
				var planName = $('#Plan-planName2').combobox('getText');
			}
			if (planName == "") {
