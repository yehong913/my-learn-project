									...
									if(jsonResult.obj!=null && jsonResult.obj.length>0) {
										for(var i=0; i<jsonResult.obj.length; i++) {
											...
											if(jsonResult.obj[i].title == '参考' && '${isKlmPluginValid}' == 'true'){
												/* $('#tt').tabs('add',{
													...
													id: 'cellReference',
													selected: 0,
													href: 'taskFlowResolveController.do?goTab&tabIndex=3&cellId=${cellId}&parentPlanId=${parentPlanId}&isEnableFlag=${isEnableFlag}&nameStandardId=${nameStandardId}'
												}); */
											}else{
												$('#tt').tabs('add',{
										...
										/* if('${isKlmPluginValid}' == 'true') {
											$('#tt').tabs('add',{
	                                            ...
	                                            id: 'cellReference',
	                                            selected: 0,
	                                            href: 'taskFlowResolveController.do?goTab&tabIndex=3&cellId=${cellId}&parentPlanId=${parentPlanId}&isEnableFlag=${isEnableFlag}'
	                                        });
										} */
				...
				success : function(data) {
					...
					if(d.success == true){
						...
						if (baseInfoId != undefined && baseInfoId != '') {
							...
							index0Tab.panel('refresh', 'taskFlowResolveController.do?goTab&tabIndex=1&cellId=${cellId}&parentPlanId=${parentPlanId}&isSave=true&change='+change);
						/* 	tab = $("#tt").tabs('getTab','<spring:message code="com.glaway.ids.pm.project.plan.baseinfo"/>');
							tab.panel('refresh','taskFlowResolveController.do?goTab&tabIndex=1&cellId=${cellId}&parentPlanId=${parentPlanId}');
							tab = $("#tt").tabs('getTab','<spring:message code="com.glaway.ids.pm.project.plan.input"/>');
							tab.panel('refresh','taskFlowResolveController.do?goTab&tabIndex=2&cellId=${cellId}&parentPlanId=${parentPlanId}');
							tab = $("#tt").tabs('getTab','<spring:message code="com.glaway.ids.pm.project.plan.output"/>');
							tab.panel('refresh','taskFlowResolveController.do?goTab&tabIndex=4&cellId=${cellId}&parentPlanId=${parentPlanId}'); */
...
<div border="false" class="easyui-panel div-msg" id="baseInfo" fit="true">
	<div class="easyui-panel" fit="true">
		<fd:form id="baseInfoFrm">
			...
			<fd:inputDate id="planStartTime" title="{com.glaway.ids.pm.project.task.taskStartTime}" value='${plan_.planStartTime}' onChange="workTimeLinkage('planStartTime')" editable="false" readonly="true" disabled="true"/>
			...
			<fd:inputDate id="planEndTime" title="{com.glaway.ids.pm.project.task.taskEndTime}" value='${plan_.planEndTime}' editable="false" onChange="workTimeLinkage('planEndTime')" readonly="true" disabled="true"/>
...
<fd:dialog id="preposePlanDialog" width="800px" height="580px" modal="true" title="{com.glaway.ids.pm.project.task.selectOutPrepose}"  zIndex="4300">
