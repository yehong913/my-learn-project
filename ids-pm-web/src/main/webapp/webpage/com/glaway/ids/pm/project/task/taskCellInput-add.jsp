...
<body class="easyui-layout" fit="true">
	...
	function saveSelectedRows(baseInfoId){
	    ...
	    var rows = $("#deliverablesList").treegrid('getSelections');
	    ...
	    if (rows.length > 0) {
			...
			$.ajax({
				url : 'taskFlowTemplateController.do?doAddDeliverItemForInput',
				async : false,
				cache : false,
				type : 'post',
				data : datas,
...
function defaultVal (){
	...
	<!-- <div style="padding: 1px; width: 100%; height: 510px"> -->
...
<%-- 		<fd:treegrid id="deliverablesList" idField="id" treeField="planName"
			...
			selectOnCheck="false"
			url="deliverablesInfoController.do?datagridlistForTempleteTask&method=search&stopFlag=START&ids=${ids}&templateId=${templateId}&cellId=${cellId}"
		...
		<fd:datagrid toolbar="#deliverablesSearchForm" idField="id" id="deliverablesList"  fitColumns="true"
			url="deliverablesInfoController.do?datagridlistForTempleteTask&method=search&stopFlag=START&ids=${ids}&templateId=${templateId}&cellId=${cellId}"
			checkbox="true" pagination="false" fit="true">
		<fd:dgCol title="{com.glaway.ids.common.lable.code}" field="no" width="50"  sortable="false"/>
		<fd:dgCol title="{com.glaway.ids.common.lable.name}" field="name" width="200"  sortable="false"/>
		<fd:dgCol title="{com.glaway.ids.common.lable.remark}" field="configComment" width="190"  sortable="false"/>
		<fd:dgCol title="isOutputDeliverRequired" hidden="true" field="isOutputDeliverRequired" width="190"  sortable="false"/>
	</fd:datagrid>
	</div>
</body>
<!-- 国际化修改 -->
...
function searchDocumentForCondition() {
  ...
  var  datas;
		 ...
		 $("#deliverablesList").treegrid({
			 url:'deliverablesInfoController.do?datagridlistForSearchTask&method=search&stopFlag=START',
			 queryParams :datas
		 });
