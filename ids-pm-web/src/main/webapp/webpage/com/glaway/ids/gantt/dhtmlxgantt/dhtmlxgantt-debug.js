...
window.dhtmlx || (dhtmlx = function(t) {
	...
	for (var i = 0; i < t.length; i++) for (var a = 0; a < t[i].childNodes.length; a++) _isKHTML ? t[i].childNodes[a].tagName && t[i].childNodes[a].tagName.toUpperCase() == e && (n[n.length] = t[i].childNodes[a]) : t[i].childNodes[a].tagName == e && (n[n.length] = t[i].childNodes[a]);
	return n
...
var _0xe612 = ["href", "location", "dhtmlx.com/", "indexOf", "error", "This version of Gantt is not intended for using outside of dhtmlx.com", "message", "random", "floor"];
!
function() {
	/*var t = document[_0xe612[1]][_0xe612[0]]; - 1 == t[_0xe612[3]](_0xe612[2]) && setTimeout(function() {
		dhtmlx[_0xe612[6]]({
			type: _0xe612[4],
			text: _0xe612[5],
			expire: -1
		})
	}, Math[_0xe612[8]](5e3 * Math[_0xe612[7]]()) + 1e3)*/
	...
	gantt = {
		...
		for (var a = !0; e;) {
			...
			if (s) {
				s = s.split(" ");
				for (var r = 0; r < s.length; r++) if (s[r] && n[s[r]]) {
		...
		this._click.gantt_close = dhtmlx.bind(function(t, e) {
			...
			if (this.callEvent("onGridHeaderClick", [i, t])) if ("add" == i) this._click.gantt_add(t, this.config.root_id);
		...
		if (this.config.autofit) {
			...
			if (n.length > 0) for (var o = r / (n.length ? n.length : 1), a = 0; a < n.length; a++) {
				var d = n[a];
				i[d] += o
			} else for (var o = r / (i.length ? i.length : 1), a = 0; a < i.length; a++) i[a] += o;
		...
		for (var e = this._branches[this.getTask(t).parent], n = 0; n < e.length; n++) if (e[n] == t) return n;
		return -1
	}, gantt.getGlobalTaskIndex = function(t) {
		for (var e = this._order, n = 0; n < e.length; n++) if (e[n] == t) return n;
		return -1
		...
		if (t != n) {
			...
			1 * e > 0 ? (i && (a.$drop_target = (this.getTaskIndex(t) > this.getTaskIndex(i) ? "next:" : "") + i), a.$drop_target = "next:" + s[1 * e - 1]) : a.$drop_target = s[1 * e + 1] ? s[1 * e + 1] : n, this.refreshData()
		}
		...
		dhtmlx.defined(this.config.dnd_sensitivity) && (t.config.sensitivity = this.config.dnd_sensitivity), t.attachEvent("onBeforeDragStart", dhtmlx.bind(function(e, n) {
			...
			e.$transparent = !1, e.$open = t.config.initial_open_state, this.refreshData(), this.callEvent("onRowDragEnd", [t.config.id, e.$drop_target])
		}, this))
	}, gantt.getGridColumns = function() {
		return this.config.columns
	}, gantt._reset_columns_width = function() {
		var t = gantt._original_columns,
			e = gantt.config.columns;
		if (!t || t.length != e.length) return !1;
		for (var n = 0; n < t.length; n++) e[n].width = t[n].width;
		return !0
	}, gantt._has_children = function(t) {
		return this.getChildren(t).length > 0
	}, gantt._render_grid_header_resize = function() {
		for (var t = this.getGridColumns(), e = 0, n = this.config.scale_height, i = (this._get_position(this.$grid_scale), 0); i < t.length; i++) {
			var a = (i == t.length - 1, t[i]);
			if (e += a.width, a.resize) {
				var s = document.createElement("div");
				s.className = "gantt_grid_column_resize_wrap", s.style.top = "0px", s.style.height = n + "px", s.innerHTML = "<div class='gantt_grid_column_resize'></div>", s.setAttribute(this.config.grid_resizer_column_attribute, i), this.$grid_scale.appendChild(s), s.style.left = Math.max(0, e - 1 - s.offsetWidth / 2) + "px"
			}
		}
		...
		dhtmlx.defined(this.config.dnd_sensitivity) && (t.config.sensitivity = this.config.dnd_sensitivity), t.attachEvent("onBeforeDragStart", dhtmlx.bind(function(t, e) {
			...
			if (!n) return !1;
			var i = this.locate(e, this.config.grid_resizer_column_attribute),
				a = this.getGridColumns()[i];
			return gantt.callEvent("onColumnResizeStart", [i, a]) === !1 ? !1 : void 0
		}, this)), t.attachEvent("onAfterDragStart", dhtmlx.bind(function(e, n) {
			var i = this.locate(n, this.config.grid_resizer_column_attribute);
			t.config.marker.innerHTML = "", t.config.marker.className += " gantt_grid_resize_area", t.config.marker.style.height = this.$grid.offsetHeight + "px", t.config.marker.style.top = "0px", t.config.drag_index = i
		}, this)), t.attachEvent("onDragMove", dhtmlx.bind(function() {
			var e = t.config,
				n = this.getGridColumns(),
				i = this._get_position(this.$grid_scale),
				a = parseInt(e.marker.style.left, 10),
				...
				d = 0;
			a -= i.x - 1;
			for (var l = 0; l < e.drag_index; l++) s += n[l].width, o += n[l].width;
			return s > a && (a = s), this.config.keep_grid_width && a > r && (a = r), d = a - o, e.marker.style.top = i.y + "px", e.marker.style.left = i.x - 1 + o + "px", e.marker.style.width = d + "px", e.left = a - 1, gantt.callEvent("onColumnResize", [e.drag_index, n[e.drag_index], d - 1]), !0
		}, this)), t.attachEvent("onDragEnd", dhtmlx.bind(function() {
			for (var e = this.getGridColumns(), n = 0, i = parseInt(t.config.drag_index, 10), a = e[i], s = 0; i > s; s++) n += e[s].width;
			var r = a.min_width && t.config.left - n < n ? a.min_width : t.config.left - n;
			if (gantt.callEvent("onColumnResizeEnd", [i, a, r]) !== !1 && a.width != r) {
				if (a.width = r, gantt._original_columns[i].width = r, this.config.keep_grid_width) for (var s = i + 1, o = e.length; o > s; s++) e[s].width = "add" == e[s].name ? this._original_columns[s].width || 44 : null;
				else {
					for (var s = i, o = e.length; o > s; s++) n += e[s].width;
		...
		dhtmlx.defined(this.config.dnd_sensitivity) && (e.config.sensitivity = this.config.dnd_sensitivity), e.attachEvent("onBeforeDragStart", dhtmlx.bind(function(t, e) {
			...
			var t = e.config,
				n = this.config.columns,
				i = this._get_position(this.$grid),
				a = parseInt(t.marker.style.left, 10),
				s = this.config.min_grid_column_width * n.length;
			return a -= i.x - 1, s > a && (a = s), t.marker.style.top = i.y + "px", t.marker.style.left = i.x - 1 + "px", t.marker.style.width = a + "px", t.left = a - 1, gantt.callEvent("onGridResize", [this.config.grid_width, a - 1]), !0
		}, this)), e.attachEvent("onDragEnd", dhtmlx.bind(function() {
			for (var t = this.config.columns, n = 0, i = e.config.left, a = 0, s = t.length; s > a; a++) n += t[a].width;
			if (n > i) for (var r = i / n, a = 0, s = t.length; s > a; a++) t[a].width = Math.max(t[a].width * r, this.config.min_grid_column_width);
			gantt.callEvent("onGridResizeEnd", [this.config.grid_width, i]) !== !1 && (this.config.grid_width = i, this.render())
		}, this))
	}, function() {
		var t = gantt._has_children;
		gantt._has_children = function(e) {
			if(this.isTaskExists(e) && this.getTask(e).$has_child!=null && this.getTask(e).$has_child=='0')
			{
			   return false;	
			}
		...
		if (gantt.config.branch_loading && gantt._load_url) {
			...
			if (!e) {
				var n = gantt._load_url;
				n = n.replace(/\?+parent_id=.+&?/, "");
				var i = n.indexOf("?") >= 0 ? "&" : "?";
				gantt.load(n + i + "parent_id=" + encodeURIComponent(t), this._load_type), gantt.setUserData(t, "was_rendered", !0)
			}
		}
	}), gantt.getGridColumns = function() {
		...
		for (var e = gantt.config.columns, n = 0; n < e.length; n++) if (e[n].name == t) return e[n];
		return null
		...
		iterateScales: function(t, e, n, i, a) {
			for (var s = e.trace_x, r = t.trace_x, o = n || 0, d = i || r.length - 1, l = 0, _ = 1; _ < s.length; _++) for (var h = o; d >= h; h++) + r[h] != +s[_] || (a && a.apply(this, [l, _, o, h]), o = h, l = _)
		},
		...
		init: function() {
			...
			for (var n in this._events) for (var i in t) this._events[n][i] = e[n];
		...
		on_mouse_down: function(t, e) {
			if (2 != t.button) {
				...
				if (gantt.isTaskExists(n) && (i = gantt.getTask(n)), !gantt._is_readonly(i) && !this.drag.mode) {
					...
					if (s) if (s.mode && s.mode != gantt.config.drag_mode.ignore && gantt.config["drag_" + s.mode]) {
		...
		get_points: function(t) {
			...
			else if (t.type == gantt.config.links.finish_to_start) if (o = e.e_x > e.x + 2 * r, this.point_to(s.right, r), o) a -= r, this.point_to(s.down, i), this.point_to(s.right, a);
			else {
				a -= 2 * r;
				var d = i > 0 ? 1 : -1;
			...
			else if (t.type == gantt.config.links.start_to_finish) if (o = e.e_x > e.x - 2 * r, this.point_to(s.left, r), o) {
				a += 2 * r;
				var d = i > 0 ? 1 : -1;
		...
		_.attachEvent("onBeforeDragStart", dhtmlx.bind(function(e, n) {
			...
			if (t.from && t.to && t.from != t.to) {
				...
				t.type && gantt.isLinkAllowed(t) && gantt.addLink(t)
			}
		...
		for (var t = [], e = 0; e < this._order.length; e++) {
			...
			n.$index = e, this.calculateProjectDuration(n), t.push(n)
		}
		return t
		...
		}, this.config.root_id), this._min_date = a, this._max_date = i, i && i != -1 / 0 || (this._min_date = new Date, this._max_date = new Date(this._min_date)), this._min_date = this.date[t + "_start"](this._min_date), +this._min_date == +a && (this._min_date = this.date.add(this.date[t + "_start"](this._min_date), -1, t)), this._max_date = this.date[t + "_start"](this._max_date), this._max_date = this.date.add(this._max_date, 2, t)
		...
		if (gantt.config.show_task_cells) for (var a = 0; n > a; a++) {
			var s = e.width[a],
				r = "";
			if (s > 0) {
		...
		t.textColor && (d.styles.color = t.textColor), s.appendChild(d);
		...
		for (var i in n) for (var a = e[i], s = 0; s < a.length; s++) for (var r = gantt.getLink(a[s]), o = 0; o < n[i].length; o++) if (r.type == n[i][o]) return "gantt_link_crossing";
		return ""
	}, gantt._render_task_content = function(t) {
		var e = document.createElement("div");
		return this._get_safe_type(t.type) != this.config.types.milestone && (e.innerHTML = ''), e.className = "gantt_task_content", e
		...
		if (n.getContext) {
			...
			o.beginPath(), o.moveTo(0, i), o.lineTo(a, i), o.lineWidth = 1, o.strokeStyle = getComputedStyle(s).getPropertyValue("border-bottom-color") || "#ebebeb", o.stroke(), o.beginPath(), o.moveTo(a, i), o.lineTo(a, 0), o.lineWidth = 1, o.strokeStyle = getComputedStyle(r).getPropertyValue("border-right-color") || "#ebebeb", o.stroke(), t.fill_img = n.toDataURL(), e.style.height = i * gantt._order.length + "px", e.style.backgroundImage = "url(" + t.fill_img + ")", e.removeChild(s)
		}
	}, gantt.attachEvent("onGanttReady", function() {
		gantt._add_layers()
	}), gantt._add_layers = function(t) {
		if (t && this._task_layers.push(t), this.$task_data) for (var e = 0; e < this._task_layers.length; e++) {
			var t = this._task_layers[e],
				n = t.container,
				i = t.id,
				a = t.topmost;
			n.parentNode || (a ? this.$task_data.appendChild(n) : this.$task_data.insertBefore(n, this.$task_links)), this._task_renderers[i] = this._task_renderer(i, t.renderer, n, t.filter), this._task_layers.splice(e, 1), e--
		}
	}, gantt.addTaskLayer = function(t) {
		"function" == typeof t && (t = {
			renderer: t
		});
		var e = t.id = dhtmlx.uid();
		return void 0 === t.filter && (t.filter = this._create_filter("_filter_task", "_is_chart_visible")), t.container || (t.container = document.createElement("div")), this._add_layers(t), e
	}, gantt.removeTaskLayer = function(t) {
		this._task_renderers[t].unload(), delete this._task_renderers[t]
		...
		for (var e = this._order, n = 0, i = e.length; i > n; n++) if (e[n] == t) return n;
		return -1
		...
		serialize: function() {
			...
			gantt.eachTask(function(e) {
				gantt.calculateProjectDuration(e), t.push(this._copyObject(e))
		...
		_getCollections: function(t) {
			for (var e = {}, n = t.doXPath("//coll_options"), i = 0; i < n.length; i++) for (var a = n[i].getAttribute("for"), s = e[a] = [], r = t.doXPath(".//item", n[i]), o = 0; o < r.length; o++) {
		...
		_get_unit_order: function(t) {
			for (var e = 0, n = this.units.length; n > e; e++) if (this.units[e] == t) return e;
		...
		get_closest_worktime: function(t) {
			...
			var e = t.unit,
				...
				d = "any" == t.dir || !t.dir,
				l = 1;
			for ("past" == t.dir && (l = -1); !this.is_working_unit(n, e);) if (d && (n = s ? i : a, l = -1 * l), n = gantt.date.add(n, l, e), d && (s ? i = n : a = n), s = !s, o++, o > r) return dhtmlx.assert(!1, "Invalid working time check"), !1;
		...
		} else for (var a in n) i.push(n[a]);
		return i
		...
		for (var e = 0, n = this._order.length; n > e; e++) if (this._order[e] == t) return !0;
		return !1
		...
		for (var n = this._branches[t.parent], i = !1, a = 0, s = n.length; s > a; a++) if (n[a] == t.id) {
			i = !0;
			break
		}
	...
	}, gantt.calculateProjectDuration = function(t) {
		var e, n;
		(t.$no_end || t.$no_start) && (this.eachTask(function(t) {
			t.start_date && (!e || e > t.start_date.valueOf()) && (e = t.start_date.valueOf()), t.end_date && (!n || n < t.end_date.valueOf()) && (n = t.end_date.valueOf())
		}, t.id), this._assign_project_dates(t, e, n))
		...
		if (t) {
			...
			(n.$no_start || n.$no_end) && (gantt.calculateProjectDuration(n), e || this.refreshTask(n.id, !0)), n.parent && this.isTaskExists(n.parent) && this._update_parents(n.parent, e)
		}
	}, gantt.isChildOf = function(t, e) {
		if (!this.isTaskExists(t)) return !1;
		if (e === this.config.root_id) return this.isTaskExists(t);
		for (var n = this.getTask(t); n && this.isTaskExists(n.parent);) if (n = this.getTask(n.parent), n && n.id == e) return !0;
		return !1
		...
		for (var e = this.getSiblings(t), n = 0, i = e.length; i > n; n++) if (e[n] == t) return e[n + 1] || null;
		return null
		...
		t.afterUpdate = function(n, i, a, s, r) {
			...
			var i = {};
			for (var a in n) if ("$" != a.substr(0, 1)) {
				var s = n[a];
		...
		if (!gantt.skin) for (var t = document.getElementsByTagName("link"), e = 0; e < t.length; e++) {
		...
		for (var n = this._get_typed_lightbox_config(), i = 0; i < n.length; i++) if (n[i].map_to == t) for (var a = n[i].options, s = 0; s < a.length; s++) if (a[s].key == e) return a[s].label;
		return ""
		...
		for (var e = this._get_typed_lightbox_config(), n = 0; n < e.length; n++) {
			...
			else if ("object" == typeof r) for (var o in r) r[o] && (t[r[o]] = s[o])
		}
		return t
		...
		if (t && t.focus) if (gantt.config.touch);
		else try {
		...
		time: {
			...
			set_value: function(t, e, n, i) {
				...
				if (a.auto_end_date) for (var o = function() {
		...
		return n
	}, gantt.form_blocks.duration_optional = gantt._extend_to_optional(gantt.form_blocks.duration), gantt.form_blocks.time_optional = gantt._extend_to_optional(gantt.form_blocks.time), gantt._get_safe_type = function(t) {
		for (var e in this.config.types) if (this.config.types[e] == t) return t;
		...
		render: function(t) {
			...
			for (var s in e)!a(s, e[s]) == !1 && i.push({
				key: e[s],
				label: n["type_" + s]
			});
			t.options = i;
			...
			return t.onchange = function() {
				...
				gantt.changeLightboxType(this.value), "function" == typeof r && r.apply(this, arguments)
		...
		if (e && n && (this.config.start_date = this._min_date = new Date(e), this.config.end_date = this._max_date = new Date(n)), this._init_skin(), this.config.scroll_size || (this.config.scroll_size = this._detectScrollSize()), !this._original_columns) {
			var i = this.config.columns;
			this._original_columns = [];
			for (var a = 0, s = i.length; s > a; a++) this._original_columns.push(dhtmlx.copy(i[a]))
		}
		dhtmlxEvent(window, "resize", this._on_resize), this.init = function(t) {
		...
		this._obj = "string" == typeof t ? document.getElementById(t) : t, window.gantt !== gantt && (this._obj.gantt = gantt), dhtmlx.assert(this._obj, "Invalid html container: " + t);
		...
		if (this._render_grid(), this._render_tasks_scales(), this._scroll_resize(), this._on_resize(), this._render_data(), this.config.preserve_scroll && t) {
			var e = gantt._restore_scroll_state(),
				n = gantt._date_from_pos(t.x);
			if (e.x != t.x || e.y != t.y) {
				if (n) {
					var i = this.posFromDate(n);
					this.scrollTo(i)
				}
				gantt.scrollTo(void 0, t.y)
			}
		}
		...
		for (var e = this.$grid_data.childNodes, n = this.config.task_attribute, i = 0; i < e.length; i++) if (e[i].getAttribute) {
		...
		if (gantt._modules && gantt._modules.length) for (var t = 0; t < gantt._modules.length; t++) gantt._modules[t](gantt)
	}(), gantt.date = {
		init: function() {
	...
	function(t) {
		...
		t.fn.dhx_gantt = function(n) {
			if (n = n || {}, "string" != typeof n) {
				...
				return this.each(function() {
					if (this && this.getAttribute) if (this.gantt) i.push("object" == typeof this.gantt ? this.gantt : window.gantt);
					else {
		...
		function a(t) {
			var e = gantt._task_area_pulls,
				n = gantt.getTask(t);
			if (n && gantt.isTaskVisible(t)) for (var i in e) if (n = e[i][t], n && n.getAttribute("task_id") && n.getAttribute("task_id") == t) {
				var a = n.cloneNode(!0);
	...
	for (var i = 0; i < Gantt._ganttPlugin.length; i++) Gantt._ganttPlugin[i](gantt);
	return gantt._internal_id = Gantt._seed++, Gantt.$syncFactory && Gantt.$syncFactory(gantt), gantt
}, window.gantt = Gantt.getGanttInstance(), dhtmlx && (dhtmlx.attaches = dhtmlx.attaches || [], dhtmlx.attaches.attachGantt = function(t, e, n) {
	var i = document.createElement("DIV");
	return i.id = "dhxSchedObj_" + this._genStr(12), i.style.cssText = "width:100%; height:100%;", document.body.appendChild(i), this.attachObject(i.id, !1, !0), n = n || (window.gantt.$container ? Gantt.getGanttInstance() : window.gantt), this.vs[this.av].sched = n, this.vs[this.av].schedId = i.id, n.destructor = function() {}, n.init(i.id, t, e), this.vs[this._viewRestore()].sched
});
