...
<div class="easyui-layout" id="document" fit="true">
	...
	<div class="know_search" id="older">
		<p>
			<input type="text" class="search_text" id="input" /><input
				type="button" value="<spring:message code='com.glaway.ids.common.btn.retrieval'/>" class="search_btn" onclick="goSearch()"/>
		</p>
	</div>
	<div style="height:230px">
		<fd:datagrid id="referenceList" fitColumns="true"  pagination="true"  onLoadSuccess="hideOperateAreaColumn2" height="100%" idField="id" fit="true"
			url="taskFlowResolveController.do?getReferenceList&taskId=${taskId}&planId=${planId}&parentPlanId=${parentPlanId}&type=flowResolve" >
			<fd:dgCol title="{com.glaway.ids.common.lable.operation}" field="deleteReferenceOperate2" formatterFunName="deleteReferencefun2" width="60"></fd:dgCol>
...
/* 	$("#"+'viewKnowledgeDetailCftct').lhgdialog("open","url:"+url); */
...
function goSearch(){
	...
	url ='${openUrl}&inPut='+inPut01+'&httpFlag=1&taskId=${taskId}&type=flowResolve' + '&isEnableFlag='+ $("#isEnableFlag").val(); 
/* 	$("#"+'goSearchCftct').lhgdialog("open","url:"+url); */

	searchCftctShow2(url);
}
function deleteReferencefun2(val, row, index) {
	var returnStr = '<a class="basis ui-icon-minus" style="display:inline-block;cursor:pointer;" onClick="deleteReferenceLine2(\''
        + index + '\')" title="<spring:message code='com.glaway.ids.common.btn.remove'/>"></a>';
	return returnStr;
}
function deleteReferenceLine2(index){
	...
	url = 'taskFlowResolveController.do?deleteReference';
...
function createdetailwindow2(title, url, width, height) {
	...
	createDialogBySize('showDocDetailReferenceDialog',url,width,heigth,title);
}

function hideOperateAreaColumn2(){
	if("1" == $("#isEnableFlag").val()){
		$('#referenceList').datagrid('hideColumn', 'deleteReferenceOperate2');
	}
}
</script>
<fd:dialog id="showDocDetailReferenceDialog" modal="true" zIndex="4300">
