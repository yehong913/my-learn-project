...
<body>
	<div class="easyui-layout" fit="true">
		<div region="center" style="padding: 1px;" style="height: auto">
			<div id="planTemplateList" style="padding: 3px; margin:10px; height: auto">
			<fd:searchform id="planTemplateTag" onClickSearchBtn="searchPlanTemplate2()" onClickResetBtn="tagSearchResetForPlanTemplate()">
				<fd:inputText title="{com.glaway.ids.pm.project.plantemplate.name}" maxLength="-1" name="name" id="planTemplateName" queryMode="like"/>
				<fd:inputText title="{com.glaway.ids.common.lable.creator}" maxLength="-1" name="createName" id="creatorName"/>
				...
				<fd:combobox id="PlanTemplateBizCurrent" maxLength="-1" textField="title" title="状态" name="PlanTemplate.bizCurrent" multiple="true" editable="false" prompt="全部" valueField="name" url="planTemplateController.do?lifeCycleStatus" queryMode="eq"></fd:combobox>
				</div>
			</fd:searchform>
		 	</div>
	...
	function planTemp_openDialogWPSPlans(param) {
		$.fn.lhgdialog({
			content : 'url:planTemplateDetailController.do?planTemplateDetail&planTemplateId='
					+ param,
 			lock : true,
			width : 900,
			height : 500,
