...
<script type="text/javascript">
        ...
        if (rows.length>0){
            for (var i = (rows.length-1); i >=0;i-- ){
                ...
                if (dataSourceId==dataSourceObjectId) {
                    ...
                    //判断是否为已存在的数据
                    if (rows[i].id!="") {
                        deleteObjPropertyById(rows[i].id, index);
                    }else{
                        // 删除选择行
                    ...
                    if (rows[0].control=="表格"){
                        ...
                        for (var i = (allRows.length-1); i >=index;i--) {
                            if (obj == allRows[i].objectPath&&(allRows[i].control=="列"||allRows[i].control=="表格")) {
                                if (allRows[i].id!=""){
                                    successFlag = deleteObjPropertyById(allRows[i].id, i);
                                }else {
                                    // 删除选择行
                                    $('#addObjectPropertyList').datagrid('deleteRow',i);
                                }
                        ...
                        for(var i = (rows.length-1); i >=0;i-- ){
                            ...
                            //判断是否为已存在的数据
                            if (rows[i].id!=""){
                                successFlag = deleteObjPropertyById(rows[i].id, index);
                                if (successFlag){
                                    top.tip("删除成功");
                                } else {
                                    top.tip("删除失败");
    ...
    //后台执行删除元数据
    function deleteObjPropertyById(id, index) {
        var flag = true;
        $.ajax({
