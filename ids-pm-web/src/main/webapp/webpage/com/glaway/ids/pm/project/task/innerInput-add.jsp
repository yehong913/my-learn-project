...
<body class="easyui-layout" fit="true">
    <script type="text/javascript">
		...
		function saveSelectedRowsResolve() {
			...
			var datas;
			var originObjectIds = [];
			var rows = $("#deliverablesListResolve").datagrid('getSelections');
			...
			if (rows.length > 0) {
				...
				$.ajax({
							url : 'taskFlowResolveController.do?doAddInnerInputs',
							async : false,
							cache : false,
							type : 'post',
							data : datas,
  ...
  <div region="center" style="padding: 1px;" >
        ...
        <fd:datagrid toolbar="#deliverablesSearchForm" idField="id" id="deliverablesListResolve" fitColumns="true"
            url="taskFlowResolveController.do?datagridlistForInnerInputs&useObjectId=${useObjectId}&ids=${deliverIds}&parentPlanId=${parentPlanId}&cellId=${cellId}"
...
<script type="text/javascript">
	...
	function searchDocumentForConditionResolve() {
	    ...
	    $.ajax({
            ...
            data : datas,
            success : function(data) {
                $("#deliverablesListResolve").datagrid("loadData",data);
            } 
        });
		
		$('#deliverablesListResolve').datagrid('unselectAll');
		$('#deliverablesListResolve').datagrid('clearSelections');
		$('#deliverablesListResolve').datagrid('clearChecked');
	}
