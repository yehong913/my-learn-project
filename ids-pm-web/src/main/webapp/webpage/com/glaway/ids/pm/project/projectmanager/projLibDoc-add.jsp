...
<script type="text/javascript">
	...
	$(document).ready(function() {
		$("#projLibDocListMsg").css("display","none");
		 //		$('#clear').hide();
	...
	function saveFileBasicAndUpload()
	{
		...
		if(curFlog=="1"){
	     ...
	     if(docName!= null && docName != '' && docName != undefined){
	    	 ...
	    	 tip('<spring:message code="com.glaway.ids.pm.project.projectmanager.library.docNameNotEmpty"/>');  
				return false;
	     }
	     ...
	     if(path!= null && path != '' && path != undefined){
	    	 ...
	    	 tip('<spring:message code="com.glaway.ids.pm.project.projectmanager.library.docPathNotEmpty"/>');  
				return false;	 
	     }

	    var addDeliveryStandard_docTypeName = $('#addDeliveryStandard_docTypeName').val();
	     if(addDeliveryStandard_docTypeName == null || addDeliveryStandard_docTypeName == '' || addDeliveryStandard_docTypeName == undefined){
			 tip('<spring:message code="com.glaway.ids.pm.project.projectmanager.library.docTypeNotEmpty"/>');
			 return false;
		 }
		  ...
		  for (var i = 0; i < allRows.length; i++) {
			  ...
			  docSecurityLevelFrom = docSecurityLevelFrom+","+a; 
...
//		  workTimeOnChange();
		  ...
		  $('#docSecurityLevelFrom').val(docSecurityLevelFrom);
		  }
		...
		$.ajax({
			type : "POST", 
			url : "projLibController.do?doAdd&attachmentNames="+ attachmentNames+"&dowmLoadUrls="+ dowmLoadUrls,
			async : false,
	...
	function userDefinedInfo(id){
		$("#userDefinedInfo").load("projLibController.do?goShowUserDefinedInfo&id="+id);	
	}
...
<body style="overflow-y: hidden;">
        <div style="position: absolute;top:0px;right: 18px;z-index: 99;"><fd:helpButton help="helpDoc:CreateDocument" /></div>
		<fd:form id="projLibDocAddForm" method="projectController.do?doSave" style="height:230px; overflow: auto;">
			...
			<fd:inputText id="docName" name="docName"
					title="{com.glaway.ids.common.lable.name}" required="true" validType="regExp['/^(\w|[a-zA-Z0-9]|[\u4E00-\u9FA5])*$/','包含特殊字符']"
					value="${doc.docName}" />
			...
			<c:choose>
			 ...
			 <fd:inputSearch id="addDeliveryStandard_docTypeName" required="true" name="docTypeName" title="文档类型"
	    		editable="false" searcher="selectDocTypeForProjLibDoc()" value="${deliveryStandard.docTypeName}" />
	    	<input name="docTypeId" id="addDeliveryStandard_docTypeId" type="hidden" value="${deliveryStandard.docTypeId}"/>
	    	
	    	<div id="userDefinedInfo" >
				<fd:additionalAttrValRender
					entityUri="com.glaway.foundation.rep.entity.RepFileType" mode="add"
					id="RepFileType_generalAddAttr" attrName="${deliveryStandard.docTypeId}" attrVal="${deliveryStandard.docTypeId}">
				</fd:additionalAttrValRender>
			</div>
	...
	<div id="p1" class="easyui-panel"
		title="<spring:message code="com.glaway.ids.pm.project.projectmanager.attachmentList"/>"
		style="width: 100%; height: 300px; overflow: hidden;  padding: 1px; background: #fafafa;"
		data-options="closable:false,collapsible:false,minimizable:false,maximizable:false">
		<!-- 工具栏 -->
		<div  class="easyui-panel" style="width: 100%; height:30px; padding: 0px; background: #fafafa;"
		data-options="closable:false,collapsible:false,minimizable:false,maximizable:false" >
		...
		<div  class="easyui-panel" style="width: 100%; height:230px; padding: 0px; background: white; overflow-x: hidden;"
		data-options="closable:false,collapsible:false,minimizable:false,maximizable:false" >
			<fd:datagrid  id="projLibDocList" checkbox="true"
				toolbar="#" fitColumns="true" idField="uuid" fit="true"
				pagination="false" url=""
				onDblClickRow="onClickRowBatch" onClickFunName="workTimeOnChange"
				width="100%">
