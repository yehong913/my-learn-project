...
<div class="easyui-panel" fit="true">
	...
	<input id="preposeIds" name="preposeIds" type="hidden" value="${preposeIds}">
			...
			<div id="planlisttb">
			 <fd:searchform id="seachPerposeTag"  onClickSearchBtn="searchPerpose()" onClickResetBtn="tagsearchPerposeReset()">
				...
				<fd:combobox id="searchBizCurrent" textField="title" title="{com.glaway.ids.common.lable.status}" name="Plan.bizCurrent"  editable="false" valueField="name" prompt="全部"
					url="planController.do?statusListForProblem" queryMode="in" multiple="true"></fd:combobox>	
			...
			<fd:datagrid id="planlist" checkbox="true" toolbar="#planlisttb" fitColumns="true"  pagination="false"
				url="planController.do?searchDatagridForProblem&id=${planId}&projectId=${projectId}" idField="id" fit="true" onLoadSuccess="selectIds">
...
<fd:dialog id="viewPlanTaskDialog" width="790px" height="500px"
	...
	function viewBasicLine(id) {
		var dialogUrl = '${pm_Nginx}/planController.do?goCheck&id=' + id;
	...
	function getLoadData(){
	    ...
	    if (rows.length > 0) {
			...
			$.ajax({
				url : '${riskproblems_Nginx}/riskProblemTaskController.do?doAddTask'+ '&taskId=${taskId}',
				async : false,
				cache : false,
				type : 'post',
				data : datas,
		...
		function viewPlan(id) {
			$.fn.lhgdialog({
				content : 'url:${pm_Nginx}/planController.do?goCheck&id=' + id,
				lock : true,
				width : 630,
				height : 400,
				zIndex : 5000,
		...
		function searchPerpose(){
			...
			var url = '${pm_Nginx}/planController.do?searchDatagridForProblem&projectId=${projectId}'+"&planName="+planName+"&bizCurrent="+bizCurrent+"&planLevel="+planLevel+"&userName="+userName+"&planStartTime="+planStartTime+"&planEndTime="+planEndTime+"&workTime="+workTime+"&workTime_condition="+workTime_condition;
		...
		//弹出对象对应的流程
		function openFlowPlans_(taskNumber) {
			var tabTitle = '计划工作流';
