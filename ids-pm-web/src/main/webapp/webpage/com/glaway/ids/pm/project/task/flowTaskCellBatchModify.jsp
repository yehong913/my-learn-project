...
<head>
	...
	<script>
		$(document).ready(
			function() {
				$.ajax({
					url : 'taskFlowResolveController.do?flowTaskList&parentPlanId='+$("#parentPlanId").val(),
					type : 'post',
					data : {
		...
		function saveChanges() {
			...
			if (currentIndex != undefined && !changeLink){
				...
				if(val != null && val != '' && val != undefined){
					...
					$.ajax({
						url : 'taskFlowResolveController.do?changeFlowTaskListAndSave',
						type : 'post',
						data : {
							id : row.id,
						...
						success : function(data) {
							...
							if(d.success){
								...
								$('#batchModifyEditList').datagrid('selectRow', wrongIndex).datagrid('beginEdit', wrongIndex);
								editIndex = wrongIndex;
				...
				if (rows.length > 0) {
					...
					$.ajax({
						url : 'taskFlowResolveController.do?doBatchSaveBasicInfo',
						type : 'post',
						data : {
							ids : ids.join(','),
							owners : owners.join(','),
		...
		function workTimeOnChange(){
			...
			if(val != null && val != '' && val != undefined){
				$.ajax({
					url : 'taskFlowResolveController.do?changeFlowTaskList',
					type : 'post',
					data : {
						id : row.id,
						projectId : row.projectId,
...
<body>
	...
	<c:if test="${isStandards == 'ok'}">
		<fd:datagrid toolbar="#tb" idField="id" id="batchModifyEditList" pagination="false" fit="true" fitColumns="true"  checkbox="true"  onDblClickRow="onClickRowBatch" onClickFunName="workTimeOnChange">
			...
			<fd:dgCol title="{com.glaway.ids.pm.project.task.taskname}" field="planName" width="200" editor="{type:'combobox',options:{
			                    url:'taskFlowResolveController.do?standardValue',
					            method:'get',
					            valueField:'name',
					            textField:'name',
			...
			<fd:dgCol title="{com.glaway.ids.common.lable.owner}" field="owner" width="100" formatterFunName="userNameShow"
					 editor="{type:'combobox',options:{
			                    url:'taskFlowResolveController.do?projectMembers&parentPlanId=${parentPlanId}',
					            method:'get',
					            valueField:'id',
					            textField:'realName',
					            panelMaxHeight:'150px'
    ...
    <c:if test="${isStandards == 'nook'}">
    	<fd:datagrid toolbar="#tb" idField="id" id="batchModifyEditList" pagination="false" fit="true" fitColumns="true"  checkbox="true"  onDblClickRow="onClickRowBatch" onClickFunName="workTimeOnChange">
			<fd:dgCol title="{com.glaway.ids.pm.project.task.taskname}" field="parentPlanName" width="200" hidden="true" sortable="false"/>
