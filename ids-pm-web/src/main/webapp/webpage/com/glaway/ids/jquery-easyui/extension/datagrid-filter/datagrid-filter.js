(function($){
	...
	$.fn.treegrid.methods.loadData = function(jq, data){
		...
		var ret = oldLoadDataMethod2.call($.fn.treegrid.methods, jq, data);
		return ret;
	};
	...
	$.extend($.fn.datagrid.defaults.filters, {
		numberRange: {
	        init: function(container, options){
	            var c = $('<div style="display:inline-block"><input type="text" class="d1"></input>~<input type="text" class="d2"></input></div>').appendTo(container);
	            c.find('.d1,.d2').numberbox({onChange:function(value){
	            	var v = $(this).parent().children(".d1,.d2");
	            	var v1 = $(v[0]).numberbox('getValue');
	            	var v2 = $(v[1]).numberbox('getValue');
	            	options.onChange(v1 + ":" + v2);
	            }}).filter = this;
	            return c;
	        },
	        destroy: function(target){
	            $(target).find('.d1,.d2').numberbox('destroy');
	        },
	        getValue: function(target){
	            var d1 = $(target).find('.d1');
	            var d2 = $(target).find('.d2');
	            return d1.numberbox('getValue') + ':'+d2.numberbox('getValue');
	        },
	        setValue: function(target, value){
	            var d1 = $(target).find('.d1');
	            var d2 = $(target).find('.d2');
	            var vv = value.split(':');
	            d1.numberbox('setValue', vv[0]);
	            d2.numberbox('setValue', vv[1]);
	        },
	        resize: function(target, width){
	            $(target)._outerWidth(width)._outerHeight(22);
	            $(target).find('.d1,.d2').numberbox('resize', width/2 - 5);
	        }
	    },
	    dateRange: {
	        init: function(container, options){
	            ...
	            c.find('.d1,.d2').datebox({
	            	onSelect:function(value){
	            		if(options.onSelect){
	            			var v = $(this).parent().children(".d1,.d2");
	            			var v1 = $(v[0]).datebox('getValue');
	            			var v2 = $(v[1]).datebox('getValue');
	            			options.onSelect(v1 + "::" + v2);
	            		}
	...
	$.fn.datagrid.defaults.operators = {
		...
		betweenDate : {
			...
			isMatch : function (source,value){
				...
				var ret1;
				var ret2;
				var temp = source.substring(0,source.indexOf(" "));
				var s = Date.parse(temp);
				if(isNaN(d1)){
					ret1 = true;
				}else{
					ret1 = d1 <= s;
	...
	function resizeFilter(target, field, width){
		...
		var ff = field ? header.find('.datagrid-filter[name="'+field+'"]') : header.find('input.datagrid-filter');
		ff.each(function(){
			...
			if (width != undefined){
				...
				this.filter.resize(this, col.boxWidth - btn._outerWidth());
			}
		});
	}
