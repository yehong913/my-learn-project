...
<head>
	<c:if test="${! empty fromType && fromType == 'tabTemplate'}">
	...
	<div region="center" style="padding: 1px;">
		...
		<fd:datagrid checkbox="false" toolbar="#projectTool" checked="true"
			...
			<fd:dgCol title="{com.glaway.ids.pm.project.projectmanager.bizCurrent}"
				field="bizCurrent" formatterFunName="viewBizCurrent" width="80"/>
...
<%--
		...
		<fd:dialog id="startAndSubmitDialog" width="800px" height="260px"
...
<script
	src="webpage/com/glaway/ids/pm/project/projectmanager/projectList.js"></script>
<script type="text/javascript">
	...
	function viewProject(val, row, value) {
		...
		return '<a href="javascript:void(0)" onclick="viewProjectDetail(\'' + row.id
	 ...
	 * @returns {String}
	 */
	function processFunction(value,row,index) {
		if (value) {
			var s = '<div style="width:100%;border:1px solid #dee5e7">' +
					'<div style="width:' + value + '%;background:#49a9ee;color:#000000;padding-left:4px;word-wrap: normal; word-break: initial;white-space: nowrap">' + value + '%' + '</div>'
			'</div>';
			return s;
		} else {
			var value1 = '0.00';
			var s1 = '<div style="width:100%;border:1px solid #dee5e7; position:relative; height:18px;">' +
					'<div style="width:' + value1 + '%;background:#49a9ee;color:#000000;padding-left:4px; height:18px;word-wrap: normal; word-break: initial;white-space: nowrap"></div><div style="position:absolute; top: 0px; left:4px">' + value1 + '%' +'</div>'
			'</div>';
			return s1;
		}
	...
	function viewProjectDetail(id) {
		var teamId = '';
		try{
			parent.loadPage("#right_page_panel",
					"${webRoot}/projectController.do?viewDetailBaseInfo&id=" + id + "&teamId=" + teamId
