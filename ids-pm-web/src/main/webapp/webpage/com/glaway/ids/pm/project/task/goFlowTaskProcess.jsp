...
<body>
	<div id="viewTaskFlow" fit="true" style="width:1440px;height:600px;">
		...
		<input type="hidden" id="oldParentPlanId" name="oldParentPlanId" value="${oldParentPlanId}" />
	...
	<script type="text/javascript">
		$(function($) {
			...
			var oldParentPlanId = $("#oldParentPlanId").val();
			var status = $("#status").val();		
			if("ORDERED" == status){
				url = "taskFlowResolveController.do?flowResolveEditorForViewChangeEditor&newDate="+newdate+"&formId=${formId}&id="+parentPlanId+"";
			}
			else{
			...
			$.ajax({
				...
				success : function(data) {
					...
					/*var url = "${rdflowWeb_Nginx}/webpage/com/glaway/ids/rdtask/workFlow/examples/editors/workfloweditor.jsp?isEnableFlag="*/
					var url = "${rdflowWeb_Nginx}/rdflowCommonController.do?goOpenEditorPage&isEnableFlag="
							+"2&parentPlanId=${oldParentPlanId}&status=${status}&newDate="+newdate+"&userId=${userId}";
