...
<script type="text/javascript">
	function saveTemplate() {
		...
		$
				.ajax({
					url : 'projectLibTemplateController.do?doUpdate',
					type : 'post',
					data : {
						name : name,
						remark : remark,
...
<body>
	<!-- 表单区域 -->
	<fd:form id="projectLibUpdForm">
		<fd:inputText id="name" name="name"
			title="{com.glaway.ids.pm.config.projectLibTemplate.projectLibTemplateName}"
			maxLength="30" required="true" value="${name }"></fd:inputText>
		<fd:inputTextArea id="remark"
			title="{com.glaway.ids.common.lable.remark}" value="${remark }"
			name="remark" maxLength="200"></fd:inputTextArea>
